...
export default function Promo12CuotasSection() {
  ...
  if (loading) {
    return (
      <section className="py-20 text-white" style={{ background: 'linear-gradient(135deg, #140a08, #2d1512, #140a08)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <p className="text-xl text-stone-300">Cargando productos promocionales...</p>
          </div>
        </div>
      </section>
    )
  ...
  return (
    <section
      ...
      style={{ background: 'linear-gradient(135deg, #140a08, #2d1512, #1a0f0c)' }}
    >
      {/* Fondo animado */}
      <div className="absolute inset-0 opacity-10">
        <div className="absolute top-20 left-10 w-32 h-32 bg-amber-900/50 rounded-full blur-3xl animate-float"></div>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {promoProducts.length === 0 ? (
          <div className="text-center">
            <p className="text-xl text-stone-300">No hay productos disponibles {planInfo ? (planInfo.cuotas === 1 ? 'en oferta de contado' : `en ${planInfo.cuotas} cuotas`) : 'promocionales'}</p>
          </div>
        ) : (
          <div className="py-4">
            <Carousel
